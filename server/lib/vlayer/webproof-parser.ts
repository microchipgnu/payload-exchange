...
function queryJsonPath(input: JsonInput, path: string): unknown {
  ...
  try {
    ...
    throw new Error(
      `JMESPath error for path "${path}": ${(error as Error).message}`,
    );
  }
}
...
function assertBigIntInteger(value: unknown, path: string): bigint {
  if (typeof value === "number") {
    if (!Number.isFinite(value) || !Number.isInteger(value)) {
      throw new Error(
        `Expected integer at path "${path}", got number ${value}`,
      );
    }
    if (!Number.isSafeInteger(value)) {
      throw new Error(
        `Integer at path "${path}" exceeds JS safe range; encode it as a string to preserve precision.`,
      );
    }
    return BigInt(value);
  }
  ...
  if (typeof value === "string") {
    ...
    if (!/^[+-]?\d+$/.test(trimmed)) {
      throw new Error(
        `Expected integer-like string at path "${path}", got ${JSON.stringify(value)}`,
      );
    }
    try {
      ...
      throw new Error(
        `Cannot parse bigint at path "${path}": ${(error as Error).message}`,
      );
    }
  }

  throw new Error(
    `Expected integer (number or string) at path "${path}", got ${describe(value)}`,
  );
}
...
function parseDecimalParts(
  ...
  if (exponentIndex !== -1) {
    ...
    if (!/^[+-]?\d+$/.test(exponentString)) {
      throw new Error(
        `Invalid exponent in number at path "${path}": ${stringValue}`,
      );
    }
  ...
  let integerPart = decimalIndex !== -1 ? base.slice(0, decimalIndex) : base;
  const fractionalPart =
    decimalIndex !== -1 ? base.slice(decimalIndex + 1) : "";

  if (!/^\d*$/.test(integerPart) || !/^\d*$/.test(fractionalPart)) {
    throw new Error(
      `Invalid numeric characters at path "${path}": ${stringValue}`,
    );
  }
 ...
 */
export function jsonGetFloatAsInt(
  input: JsonInput,
  path: string,
  precision: number,
): bigint {
  ...
  if (typeof value !== "string" && typeof value !== "number") {
    throw new Error(
      `Expected decimal (string or number) at path "${path}", got ${describe(value)}`,
    );
  }
...
function findHttpResponse(text: string): ParsedResponse | null {
  ...
  const bodyStart = responseIndex + headerEnd + 4;
  ...
  if (contentLengthHeader) {
    ...
    if (Number.isFinite(contentLength)) {
      bodyText = text.slice(bodyStart, bodyStart + contentLength);
    }
  }
...
function findHttpRequest(text: string): ParsedRequest | null {
  const requestMatch =
    /(GET|POST|PUT|PATCH|DELETE)\s+(https?:\/\/[^\s]+)\s+HTTP\/(1\.[01]|2)/.exec(
      text,
    );
  ...
  const start = requestMatch.index ?? 0;
